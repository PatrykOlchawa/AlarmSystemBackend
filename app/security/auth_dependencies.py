...
def get_client_token_payload(
    token: str = Depends(oauth2_scheme)
):
    try:
        payload = jwt_handler.decode_access_token(token)
        return ClientTokenPayload(**payload)
...
def get_current_client(
    payload: ClientTokenPayload = Depends(get_client_token_payload),
    service: ClientService = Depends(get_client_service)
):
    client = service.get_by_client_id(payload.client_id)

    if not client.enabled:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Client not enabled"
        )
    return client